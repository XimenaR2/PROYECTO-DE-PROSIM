...
// ─────────────────────────────────────────────────────────────────────────────
// QUÉ APLICA AL EXPORTAR:
//   ✅ Filtros CSS  (B&W, Sepia, Vivid, Fade, Cool, Warm, Dramatic, Vintage, Night)
//   ✅ Ajustes      (brillo, contraste, saturación, desenfoque)
//   ✅ Rotación     (0°, 90°, 180°, 270°)
//   ✅ Texto        (todos los overlays con posición, color y tamaño)
//   ✅ Recorte      (si hay cropOverlay activo)
//
// FORMATO DE SALIDA:
//   → MP4/H.264  si el navegador soporta VideoEncoder (Chrome 94+, Safari 16.4+)
//   → WebM/VP8   como fallback (Firefox, navegadores más viejos)
// ─────────────────────────────────────────────────────────────────────────────

document.getElementById("btnExportar")?.addEventListener("click", exportVideoCanvas);

async function exportVideoCanvas() {
  if (!currentVideo) {
    showToast("⚠️ Importa o selecciona un video primero");
    return;
  }

  if (!currentVideo.videoWidth) {
    showToast("⏳ El video aún está cargando, intenta en un momento");
    return;
  }

  // Si currentFile aún no está listo (video default cargando en segundo plano)
  // esperar hasta 8 segundos antes de continuar sin él
  if (!currentFile) {
    showToast("⏳ Preparando video para exportar…");
    await new Promise(resolve => {
      let tries = 0;
      const check = setInterval(() => {
        tries++;
        if (currentFile || tries >= 16) { clearInterval(check); resolve(); }
      }, 500);
    });
  }
  ...
  // Calcular región de recorte en píxeles reales del video
  let cropRect = null;
  if (cropActive && cropOverlay) {
    const vRect = currentVideo.getBoundingClientRect();
    const cRect = cropOverlay.getBoundingClientRect();
    const scaleX = vw / vRect.width;
    const scaleY = vh / vRect.height;
    cropRect = {
      x: Math.max(0, Math.round((cRect.left - vRect.left) * scaleX)),
      y: Math.max(0, Math.round((cRect.top  - vRect.top)  * scaleY)),
      w: Math.round(cRect.width  * scaleX),
      h: Math.round(cRect.height * scaleY),
    };
    cropRect.w = Math.min(cropRect.w, vw - cropRect.x);
    cropRect.h = Math.min(cropRect.h, vh - cropRect.y);
  }

  const outW     = cropRect ? cropRect.w : vw;
  const outH     = cropRect ? cropRect.h : vh;
  const rotated90 = (currentRotation === 90 || currentRotation === 270);

  // H.264 requiere dimensiones pares
  const canvasW = Math.ceil((rotated90 ? outH : outW) / 2) * 2;
  const canvasH = Math.ceil((rotated90 ? outW : outH) / 2) * 2;

  // ── 2. Filtro CSS completo ─────────────────────────────────────────────────
  const { brightness, contrast, saturation, blur } = adjustValues;
  const baseFilter    = currentFilter !== "none" ? currentFilter + " " : "";
  const adjustFilter  = `brightness(${brightness}%) contrast(${contrast}%) saturate(${saturation}%) blur(${blur}px)`;
  const fullCSSFilter = (baseFilter + adjustFilter).trim();

  // ── 3. Preparar textos con coordenadas escaladas al canvas ─────────────────
  const vRect = currentVideo.getBoundingClientRect();
  const texts = textOverlays.map(el => {
    const eRect      = el.getBoundingClientRect();
    const relX       = (eRect.left - vRect.left) / vRect.width;
    const relY       = (eRect.top  - vRect.top)  / vRect.height;
    const scaledSize = parseInt(el.dataset.fontSize || 24) * (vw / vRect.width);
    return {
      text:  el.textContent,
      color: el.dataset.textColor || "#ffffff",
      size:  Math.round(scaledSize),
      x:     Math.round(relX * outW),
      y:     Math.round(relY * outH),
    };
  });

  // ── 4. Canvas de renderizado ───────────────────────────────────────────────
  const canvas = document.createElement("canvas");
  canvas.width  = canvasW;
  canvas.height = canvasH;
  const ctx = canvas.getContext("2d");

  // Dibuja un frame con todos los efectos aplicados
  function renderFrame() {
    ctx.save();
    ctx.clearRect(0, 0, canvasW, canvasH);
    ctx.filter = fullCSSFilter;

    if (currentRotation !== 0) {
      ctx.translate(canvasW / 2, canvasH / 2);
      ctx.rotate((currentRotation * Math.PI) / 180);
      ctx.translate(
        rotated90 ? -canvasH / 2 : -canvasW / 2,
        rotated90 ? -canvasW / 2 : -canvasH / 2
      );
    }

    if (cropRect) {
      ctx.drawImage(currentVideo, cropRect.x, cropRect.y, cropRect.w, cropRect.h, 0, 0, outW, outH);
    } else {
      ctx.drawImage(currentVideo, 0, 0, canvasW, canvasH);
    }

    ctx.restore();

    // Textos encima (sin rotación, sin filtro)
    texts.forEach(t => {
      ctx.save();
  ...
  // ── 5. Elegir ruta de exportación ─────────────────────────────────────────
  const canUseMP4 = typeof VideoEncoder !== "undefined" && typeof Mp4Muxer !== "undefined";
  const progressModal = showExportProgressModal();

  try {
    if (canUseMP4) {
      await exportAsMP4(canvas, ctx, canvasW, canvasH, renderFrame, progressModal);
    } else {
      await exportAsWebM(canvas, ctx, canvasW, canvasH, renderFrame, progressModal);
    }
  } catch (err) {
    progressModal.remove();
...
// ─────────────────────────────────────────────────────────────────────────────
// RUTA A — MP4 con VideoEncoder + mp4-muxer
// Soportado en: Chrome 94+, Edge 94+, Safari 16.4+, Android Chrome, iOS Safari
// ─────────────────────────────────────────────────────────────────────────────
async function exportAsMP4(canvas, ctx, canvasW, canvasH, renderFrame, progressModal) {
  ...
  encoder.configure({
    codec:     "avc1.42001f", // H.264 Baseline — máxima compatibilidad móvil
    width:      canvasW,
    height:     canvasH,
    bitrate:    5_000_000,
    framerate:  30,
  ...
  const FPS       = 30;
  const frameTime = 1_000_000 / FPS; // microsegundos por frame
  let frameCount  = 0;
  let rafHandle   = null;
  ...
  await new Promise(r => { currentVideo.onseeked = r; setTimeout(r, 600); });
  currentVideo.play();

  await new Promise((resolve, reject) => {
    function processFrame() {
      if (encoderError) { reject(encoderError); return; }
      if (currentVideo.ended) { resolve(); return; }

      renderFrame();

      const timestamp = Math.round(frameCount * frameTime);
    ...
    currentVideo.onended = () => { cancelAnimationFrame(rafHandle); resolve(); };
    rafHandle = requestAnimationFrame(processFrame);
  });
  ...
  progressModal.remove();
  showToast("✅ Video exportado como MP4 — compatible con iOS y Android");
}

// ─────────────────────────────────────────────────────────────────────────────
// RUTA B — WebM con MediaRecorder (fallback para Firefox y navegadores viejos)
// ─────────────────────────────────────────────────────────────────────────────
async function exportAsWebM(canvas, ctx, canvasW, canvasH, renderFrame, progressModal) {
  ...
  progressModal.remove();
  showToast("✅ Video exportado como WebM");
}
...
function downloadBlob(blob, ext) {
  ...
  const a   = document.createElement("a");
  a.href     = url;
  a.download = `video-editado-${Date.now()}.${ext}`;
...
function showExportProgressModal() {
  ...
  modal.innerHTML = `
    <div style="background:white;border-radius:20px;padding:40px;text-align:center;
                width:min(90vw,340px);box-shadow:0 30px 60px rgba(0,0,0,0.35);">
      <div style="font-size:52px;margin-bottom:14px;">🎬</div>
      <h3 style="margin:0 0 8px;color:#222;font-size:18px;">Exportando video…</h3>
      <p style="margin:0 0 6px;color:#888;font-size:13px;line-height:1.6;">
        Procesando con todos los efectos aplicados.
      </p>
      <p style="margin:0 0 20px;color:#aaa;font-size:12px;">
        ⏳ El tiempo depende de la duración del video
      </p>
      <div style="height:7px;background:#f0f0f0;border-radius:999px;overflow:hidden;">
        <div id="exportBar" style="height:100%;width:0%;
             background:linear-gradient(90deg,#33c6b8,#2ab3a6);
             border-radius:999px;transition:width 0.4s ease;"></div>
      </div>
      <p id="exportTimeLabel" style="margin:10px 0 0;font-size:12px;color:#bbb;">Iniciando…</p>
    </div>
  `;
  document.body.appendChild(modal);

  const duration  = (currentVideo?.duration || 10) * 1000;
  const startTime = Date.now();
  const bar   = modal.querySelector("#exportBar");
  const label = modal.querySelector("#exportTimeLabel");
  ...
  const ticker = setInterval(() => {
    ...
    if (label) label.textContent = remaining > 0 ? `~${remaining}s restantes` : "Finalizando…";
    if (pct >= 95) clearInterval(ticker);
  }, 400);

  const obs = new MutationObserver(() => {
    if (!document.body.contains(modal)) { clearInterval(ticker); obs.disconnect(); }
  });
